...
class calendarView(APIView):
    ...
    def get(self, request, year):
        ...
        first_monday = datetime.datetime(int(
            year), 1, 7) + datetime.timedelta(days=-datetime.datetime(int(year), 1, 7).weekday())
        ...
        for p in range(4):
            for j in [4, 4, 5]:
                ...
                for k in range(0, j):
                    ...
                    for i in range(0, 7):
                        days.append(
                            (first_monday+datetime.timedelta(days=i+(weekNumber-1)*7)))
                    week["days"] = days
                    weeks.append(week)
